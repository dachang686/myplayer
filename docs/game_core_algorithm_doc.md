...
`calcOVR(attrs, pos)` 委托 V6 位置加权拟合模型，只使用页面可见的 14 项属性和既有位置字段，不读取来源 OVR，也没有球员个人特判。主位置权重占 80%，副位置占 20%；同一属性包会因位置产生不同实时 OVR。

14 项属性按位置使用离线拟合出的非负权重直接合成为 overall；来源 OVR 仅在离线训练时充当标签，运行时不读取。CLU 与其他 13 项属性同样参与 OVR；比赛事件仍通过投射、终结、组织、防守、篮板等派生画像保留不同球员风格。全 50 属性必为 50，任一属性 +1 不会降低 OVR。
...
`player.ovr` 是唯一对外和业务使用的实时 OVR。属性变更、读档迁移和新球员生成时由 V6 公式写入；名单、合同、交易、阵容和球队综合战力只读取该字段，不在读取路径重新计算。比赛事件仍读取对应属性画像，以保留投射、盖帽、篮板等差异。现实来源 OVR 仅保存为内部 `_sourceOvr` 审计锚点，不参与用户界面或游戏决策。随机新秀只做最多 ±3 的统一平移，不使用逐属性 rollback 追逐目标；固定新秀则以审核属性为轮廓，允许为选秀档位做较大的统一平移，并记录 `_draftAttributeShift`。
