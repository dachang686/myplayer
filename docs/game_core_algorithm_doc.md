...
`calcOVR(attrs, pos)` 采用分层能力模型，不再把 13 项属性直接做一次加权平均：

1. 用与比赛模拟一致的递减曲线把原始属性转为有效能力；
2. 组合为得分、组织、防守、身体、关键球和全面性六个维度；
3. 按位置组合六维分，复合位置采用主位置 80% + 副位置 20%；
4. 用位置校准曲线映射到 40–99 OVR，维度分超过 80 后获得连续的精英段增益。

不同得分方式和防守方式会先分别计算，再按最强方式组合，因此投射核心、突破手、背身内线和护框中锋不会被无关短板简单平均掉。配置位于 `SIM_CONFIG.OVR_MODEL`。
...
控卫侧重组织与得分，中锋侧重防守、身体与全面性。现实球员的 13 项简化属性无法覆盖 2K 的投篮智商、稳定性、传球视野等完整维度，因此运行时保存审核 OVR 与初始公式分，并按 `OVR = 审核 OVR +（当前公式分 - 初始公式分）× 属性变化系数` 计算总评，最后四舍五入并限制在 `[40, 99]`。当前属性变化系数为 `1.0`。内部公式分保留未截断小数，避免高水平球员因为内部提前封顶 99 而无法继续响应属性变化。这样当前名单与审核值一致，后续成长和衰退仍由属性驱动。自建球员、生成球员和固定新秀不使用现实球员锚点，继续由属性公式直接计算，最终 OVR 同样限制在 `[40, 99]`。
