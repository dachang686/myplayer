...
> [!NOTE]
> 本文档所有论断均已与源代码逐行交叉比对核实（最后校验时间 2026-09-01）。
...
`calcOVR(attrs, pos)` 委托 V5 角色影响模型，只使用页面可见的 14 项属性，不读取来源 OVR，也没有球员个人特判。位置目前只用于其他系统的资格与发展判断，不改变 V5 overall：同一属性集在 PG–C 得到相同实时 OVR。

属性先合成为投射牵制、篮下终结、持球创造、组织、外防、护框和篮板等技能，再合成为主创、二创、中轴、外线防守者、换防者和防守中轴等角色。overall 取攻防高侧 84% 与低侧 32%，并只对完整顶级角色给有限加成；CLU 作为关键回合字段返回，但不进入 overall。全 50 属性必为 50，任一属性 +1 不会降低 OVR。
...
`player.ovr` 是唯一对外和业务使用的实时 OVR。属性变更、读档迁移和新球员生成时由 V5 公式写入；名单、合同、交易、阵容和球队综合战力只读取该字段，不在读取路径重新计算。比赛事件仍读取对应属性画像，以保留投射、盖帽、篮板等差异。现实来源 OVR 仅保存为内部 `_sourceOvr` 审计锚点，不参与用户界面或游戏决策。随机新秀只做最多 ±3 的统一平移，不使用逐属性 rollback 追逐目标；固定新秀则以审核属性为轮廓，允许为选秀档位做较大的统一平移，并记录 `_draftAttributeShift`。
