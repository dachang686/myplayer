...
`calcOVR(attrs, pos)` 委托 V5 角色影响模型，只使用页面可见的 14 项属性，不读取来源 OVR，也没有球员个人加分项。位置目前只用于其他系统的资格与发展判断，不改变 V5 overall：同一属性集在 PG–C 得到相同比赛 OVR。
...
`player.ovr` 是唯一对外和业务使用的比赛 OVR，由当前属性通过 V5 公式计算，名单、合同、交易、阵容和比赛共用该数值。现实来源 OVR 仅保存为内部 `_sourceOvr` 审计锚点，不参与用户界面或游戏决策。随机新秀只做最多 ±3 的统一平移，不使用逐属性 rollback 追逐目标；固定新秀则以审核属性为轮廓，允许为选秀档位做较大的统一平移，并记录 `_draftAttributeShift`。
