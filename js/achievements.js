...
if (!Object.values(ACHIEVEMENT_FEATURES).some(v => v)) {
  ...
  function loadData(cb) {
    ...
    if (typeof indexedDbStorageGet === 'function') {
      indexedDbStorageGet(STORAGE_KEY).then(function (raw) {
  ...
  function saveData(data, cb) {
    ...
    _cache = data;
    if (typeof indexedDbStorageSet === 'function') {
      indexedDbStorageSet(STORAGE_KEY, data).then(function () {
        if (cb) cb();
      }).catch(function () {
        if (cb) cb();
      });
  ...
  function showConquestDetail(teamCode) {
    loadData(function (data) {
      var champs = (data.championships || []).filter(function (c) {
        return c.team === teamCode;
      });
  ...
  function showConquestDetailExpanded(teamCode, champIdx) {
    loadData(function (data) {
      ...
      if (c.archetype) {
        var archCn = typeof PLAYER_ARCHETYPES !== "undefined" && PLAYER_ARCHETYPES[c.archetype] ? PLAYER_ARCHETYPES[c.archetype].cn : c.archetype;
  ...
  function generateConquestPoster() {
    loadData(function (data) {
      ...
      var H = 136 + gridH + 30 + 20; // header + title + progress + gap + grid + footer + pad
      ...
      // 渐变金字标题
      var grad = ctx.createLinearGradient(220, 0, 380, 0);
      ...
      ctx.fill();
      var gradBar = ctx.createLinearGradient(barX, 0, 480, 0);
      ...
      ctx.fillText(pct + "%", barX + fillW / 2, y + 7);
      ctx.textBaseline = "top";
      y += 30;

      // ── 球队网格 ──
      var cellW = Math.floor(112);
      var logoSize = 32;
      var cellPad = Math.floor(40);
      for (var confKey in CONQUEST_CONFERENCES) {
        var conf = CONQUEST_CONFERENCES[confKey];
        ...
        ctx.textAlign = "left";
        ctx.textBaseline = "middle";
        ctx.fillText(conf.name, 30, y + 12);

        // 右边计数
        ctx.fillStyle = Cc.gold;
        ctx.textAlign = "right";
        ctx.font = "600 11px " + Cc.fd;
        ctx.fillText(confWon + "/" + confTeams.length, 570, y + 12);
        ctx.textBaseline = "top";
        y += 28;
        ...
        for (var row = 0; row < 3; row++) {
          for (var col = 0; col < 5; col++) {
            ...
            if (hasWon) {
              ...
              ctx.fillStyle = Cc.gold;
              ctx.beginPath();
              ctx.arc(tx + 16, ty + 16, 17, 0, Math.PI * 2);
              ctx.fill();
              ctx.font = "10px sans-serif";
              ctx.textAlign = "center";
              ctx.textBaseline = "middle";
              ctx.fillText("🏆", tx + 16, ty + 16);
            } else {
            ...
            ctx.fillText(team, tx + 16, ty + logoSize + 4);
          }
          y += 48;
        }
      }
...
`;
  ...
  window.CONQUEST_API = {
    ...
    info: {
      version: "1.0",
      features: Object.keys(ACHIEVEMENT_FEATURES).filter(function (k) {
        return ACHIEVEMENT_FEATURES[k];
      }),
  ...
  // 标记已加载
  console.log("🏆 成就系统已加载, 启用模块:", Object.keys(ACHIEVEMENT_FEATURES).filter(function (k) {
    return ACHIEVEMENT_FEATURES[k];
  }).join(", "));
